...
```bash
source {{PROJECT_DIR}}/scripts/notion-api.sh && notion_query_database "{{ACTIVITY_LOGS_DB_ID}}" '{"property":"Date","date":{"equals":"<TODAY_DATE>"}}'
```
...
```bash
source {{PROJECT_DIR}}/scripts/notion-api.sh && notion_create_page '<page_body_json>'
```
...
```bash
source {{PROJECT_DIR}}/scripts/notion-api.sh && notion_append_blocks "<PAGE_ID>" '<children_json>'
```
...
```bash
source {{PROJECT_DIR}}/scripts/notion-api.sh && notion_query_database "{{DAILY_REPORT_DB_ID}}" '{"property":"日期","date":{"equals":"<TODAY_DATE>"}}'
```
