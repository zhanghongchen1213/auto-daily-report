...
curl -X POST "https://api.notion.com/v1/pages" \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "parent": {"database_id": "7efbdb06-82de-4688-834c-7a377db93077"},
    "properties": {
      "周报标题": {"title": [{"text": {"content": "'"$WEEK_START_YEAR"'年第'"$WEEK_NUMBER"'周工作周报 ('"$WEEK_START_MMDD"' - '"$WEEK_END_MMDD"')"}}]},
      "周期": {"date": {"start": "'"$WEEK_START"'", "end": "'"$WEEK_END"'"}},
      "工作领域": {"multi_select": [{"name": "领域1"}, {"name": "领域2"}]},
      "状态": {"status": {"name": "已完成"}},
      "工作日天数": {"number": 0},
      "总活动记录数": {"number": 0},
      "涉及领域数": {"number": 0},
      "摘要": {"rich_text": [{"text": {"content": "本周工作概览..."}}]}
    }
  }' 2>/dev/null
```
...
使用 Notion API 将周报内容作为 block children 追加到页面：
    ...
    "children": [
      {
        ...
        "type": "heading_2",
        "heading_2": {
          "rich_text": [{"type": "text", "text": {"content": "📋 本周工作概览"}}]
        }
      }
    ]
  }' 2>/dev/null
...
使用 Notion API 查询周报数据库，确认本周周报已成功创建。
