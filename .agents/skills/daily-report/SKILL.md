...
**数据库 ID：**
- Activity Logs: `2e0be665-a1c7-806e-8cfe-e5af8a80d291`
- 每日工作日报: `c7b19046-7ddf-44ec-b0f2-839577df4cbe`

**仓库配置来源：** `/Users/xiaozhangxuezhang/Documents/GitHub/auto-daily-report/config.json`

**当前仓库列表（3 个）：**
- `/Users/xiaozhangxuezhang/Documents/GitHub/main_control`
- `/Users/xiaozhangxuezhang/Documents/GitHub/Legbots-App`
- `/Users/xiaozhangxuezhang/Documents/GitHub/LegBots-kanban`
...
**校验点**: 如果 `total_repos_scanned` 不等于 `config.json` 中的仓库数量（当前为 3），必须报错并重新采集。
...
const result = await notionRequest(
  `/databases/2e0be665-a1c7-806e-8cfe-e5af8a80d291/query`,
  'POST',
  {
    filter: {
      property: 'Date',
...
function normalizeCodeLanguage(language) {
  ...
  if (!value || value === 'text' || value === 'plain') return 'plain text';
...
function richTextFromMarkdown(input) {
  ...
  while ((match = pattern.exec(text)) !== null) {
    if (match.index > lastIndex) {
      parts.push(...splitRichText(text.slice(lastIndex, match.index)));
    }

    const token = match[0];
    if (token.startsWith('**')) {
      parts.push(...splitRichText(token.slice(2, -2), { bold: true }));
    } else if (token.startsWith('\`')) {
      parts.push(...splitRichText(token.slice(1, -1), { code: true }));
    }
    lastIndex = pattern.lastIndex;
  }

  if (lastIndex < text.length) {
    parts.push(...splitRichText(text.slice(lastIndex)));
  }
...
function pushTableBlock(blocks, tableRows) {
  if (tableRows.length === 0) return;
  const tableWidth = Math.max(...tableRows.map(row => row.length));
  const normalizedRows = tableRows.map(row => {
    const cells = row.slice(0, tableWidth);
  ...
  blocks.push({
    ...
    table: {
      ...
      children: normalizedRows.map(row => ({
        type: 'table_row',
        table_row: {
          cells: row.map(cell => richTextFromMarkdown(cell))
        }
      }))
    }
  });
...
function appendBlock(blocks, type, richText) {
  blocks.push({
    object: 'block',
    type,
...
function markdownToNotionBlocks(markdown) {
  ...
  function flushTable() {
    if (inTable) {
      pushTableBlock(blocks, tableRows);
      inTable = false;
      tableRows = [];
    }
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmedLine.startsWith('\`\`\`')) {
      ...
      if (!inCodeBlock) {
        inCodeBlock = true;
        codeBlockLanguage = normalizeCodeLanguage(trimmedLine.slice(3));
        codeBlockContent = [];
      } else {
        inCodeBlock = false;
        blocks.push({
          ...
          code: {
            ...
            rich_text: splitRichText(codeBlockContent.join('\n'))
          }
        });
      }
      continue;
    ...
    if (trimmedLine.startsWith('|') && trimmedLine.includes('|')) {
      if (!isMarkdownTableSeparator(trimmedLine)) {
        tableRows.push(tableCellsFromLine(trimmedLine));
      }
      inTable = true;
      const nextLine = lines[i + 1]?.trim() || '';
      if (!nextLine.startsWith('|')) flushTable();
      continue;
    }
    flushTable();
    ...
    if (headingMatch) {
      ...
      appendBlock(blocks, type, richTextFromMarkdown(headingMatch[2].trim()));
      continue;
    }
    ...
    if (unorderedMatch) {
      appendBlock(blocks, 'bulleted_list_item', richTextFromMarkdown(unorderedMatch[1].trim()));
      continue;
    }

    const orderedMatch = rawLine.match(/^\s*\d+[.)]\s+(.+)$/);
    if (orderedMatch) {
      appendBlock(blocks, 'numbered_list_item', richTextFromMarkdown(orderedMatch[1].trim()));
      continue;
    }

    const quoteMatch = rawLine.match(/^\s*>\s+(.+)$/);
    if (quoteMatch) {
      appendBlock(blocks, 'quote', richTextFromMarkdown(quoteMatch[1].trim()));
      continue;
    }

    appendBlock(blocks, 'paragraph', richTextFromMarkdown(trimmedLine));
  }
...
function assertNoRawMarkdownBlocks(blocks) {
  const leaked = [];
  for (const block of blocks) {
    if (block.type !== 'paragraph') continue;
    const content = plainTextFromRichText(block.paragraph?.rich_text).trim();
    if (/^#{1,6}\s*\S/.test(content) || /^[-*]\s+\S/.test(content) || /^\d+[.)]\s+\S/.test(content)) {
      leaked.push(content.slice(0, 80));
    }
  }
  if (leaked.length > 0) {
    throw new Error('Markdown 未完全转换为 Notion 原生块，禁止写入。残留示例: ' + leaked.join(' | '));
  }
}
```
...
assertNoRawMarkdownBlocks(notionBlocks);
...
const result = await notionRequest('/pages', 'POST', {
  parent: { database_id: 'c7b19046-7ddf-44ec-b0f2-839577df4cbe' },
  properties: {
    '日报标题': {
...
- 1-6 级标题都会变为 Notion heading 块；`####标题` 这类无空格四级标题会映射为 heading_3
- 表格会变为 table 块
- 无序列表会变为 bulleted_list_item 块，数字列表会变为 numbered_list_item 块，缩进列表也必须被识别
- 代码块会变为 code 块
- 写入前必须调用 `assertNoRawMarkdownBlocks(notionBlocks)`；若 paragraph 中仍残留 `#` 标题或列表 Markdown 标记，必须停止写入并修正转换逻辑
...
```javascript
const result = await notionRequest(
  `/databases/c7b19046-7ddf-44ec-b0f2-839577df4cbe/query`,
  'POST',
  {
    filter: {
      property: '日期',
...
3. 页面正文必须包含"仓库覆盖总表"
4. 三个仓库名（main_control、Legbots-App、LegBots-kanban）必须全部在正文中出现
